...
from sm_content import SmContent
from smule_model import *
from user_form import UserForm

app = Flask(__name__)
app.config['SECRET_KEY'] = 'secret_key'
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///data/smule.db'
db = SQLAlchemy(app)
...
@app.route('/smulelist/<string:user>')
def smulelist(user):
  content   = []
  singers   = {}
  smcontent = SmContent(user, db)
  records   = smcontent.content
  logging.info(records)
  return(f"/smulelist/{user}")
...
#  file = "/mnt/d/SMULE/STORE/#{sdir}/#{sid}.m4a"
